...
{
  argparser_parse(parser, argc, (char **) argv);
  cr_expect(argparser_passed(parser, "-d") == false, "Expected -d not to be passed.");
}

Test(parsing, long_a_passed, .init=create_parser_with_options, .fini=destroy_parser)
{
  argparser_parse(parser, argc, (char **) argv);
  cr_expect(argparser_passed(parser, "--option-a"), "Expected --option-a to be passed.");
}

Test(parsing, long_b_passed, .init=create_parser_with_options, .fini=destroy_parser)
{
  argparser_parse(parser, argc, (char **) argv);
  cr_expect(argparser_passed(parser, "--option-b"), "Expected --option-b to be passed.");
}

Test(parsing, long_c_passed, .init=create_parser_with_options, .fini=destroy_parser)
{
  argparser_parse(parser, argc, (char **) argv);
  cr_expect(argparser_passed(parser, "--option-c"), "Expected --option-c to be passed.");
}

Test(parsing, long_d_not_passed, .init=create_parser_with_options, .fini=destroy_parser)
{
  argparser_parse(parser, argc, (char **) argv);
  cr_expect(argparser_passed(parser, "--option-d") == false, "Expected --option-d not to be passed.");
}
